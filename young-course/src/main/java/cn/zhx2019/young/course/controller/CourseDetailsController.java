package cn.zhx2019.young.course.controller;
...
public class CourseDetailsController {
    ...
    public ModelAndView courseDetails(int cid, HttpServletRequest request) {
        ...
        //2 获取当前课程编号对应的评价表
        Evaluation evaluation = evaluationService.getEvaluationByCourse_code(courseCode);
        ...
        List<CourseSelected> courseSelectedList = courseSelectedService.getAllRemarkByCourse_code(courseCode);
    ...
    @ResponseBody
    public String chooseCourse(int cid, int uid, String courseCode){
        //内部逻辑描述：
        //  1.对course表的操作：小于课程限选人数就course_selectnum加1，满了就返回FULL
        //  2.对course_selected表的操作：直接更改，不成功就新增（记得写入courseCode）
        String chooseCourse = courseSelectedService.chooseCourse(cid, uid, courseCode);
